...
The browser helper passes only whitelisted post/profile fields and explicitly
requested account data over the local stdin/stdout pipe. Do not read/export
cookies or tokens, access the keychain,
...
Hidden Words changes require `hidden-words add --apply`. Append to an explicitly
named custom filter, preserve its existing words/scope/duration/description, and
verify persisted state after reload. Preferences are private settings, never
public post evidence. Only `--create` may create a missing filter; no automatic
retry of Save. Keep the browser-language/version compatibility boundary explicit.
